...
{
    internal static class DESCryptography
    {
        /// <summary>
        /// Declaration of BLOCK8BYTE class
        /// </summary>
        private class BLOCK8BYTE
        {
        ...
        private const int KEY_BYTE_LENGTH = 8;
        ...
        private static bool IsValidDESKey(byte[] Key)
        {
        ...
        private static bool IsStrongDESKey(byte[] Key)
        {
        ...
        /// <returns></returns>
        private static byte[] DoPadWithString(byte[] Input, int PadLength, byte Padding)
        {
        ...
        private static void DES(byte[] bufferIn, ref byte[] bufferOut, byte[] Key, bool bEncrypt)
        {
        ...
        /// <returns></returns>
        private static bool IsValidTripleDESKey(byte[] Key)
        {
        ...
        private static void TripleDES(byte[] bufferIn, ref byte[] bufferOut, byte[] Key, bool bEncrypt)
        {
        ...
        /// <summary>
        /// Encrypt string with triple des
        /// </summary>
        /// <param name="Value"></param>
        /// <param name="KeyString"></param>
        /// <returns></returns>
        public static string TripleDESEncryptString(string keyString, string value)
        {
            string hashString = MD5Core.GetHashString(Encoding.UTF8.GetBytes(keyString));
            byte[] bytes = Encoding.UTF8.GetBytes(hashString.Substring(0, 24));
            int length = 8 - value.Length % 8;
            string str = new string(Convert.ToChar(length), length);
            value = string.Concat(value, str);
            byte[] numArray = null;
            TripleDES(Encoding.UTF8.GetBytes(value), ref numArray, bytes, true);
            return Convert.ToBase64String(numArray);
        }

        /// <summary>
        /// Descrypt string with triple des
        /// </summary>
        /// <param name="Value"></param>
        /// <param name="KeyString"></param>
        /// <returns></returns>
        public static string TripleDESDecryptString(string keyString, string value)
        {
            string hashString = MD5Core.GetHashString(Encoding.UTF8.GetBytes(keyString));
            byte[] bytes = Encoding.UTF8.GetBytes(hashString.Substring(0, 24));
            byte[] numArray = null;

            TripleDES(Convert.FromBase64String(value), ref numArray, bytes, false);
            string str = Encoding.UTF8.GetString(numArray, 0, (int)numArray.Length);
            char[] charArray = str.ToCharArray(0, str.Length);
            int num = charArray[(int)charArray.Length - 1];
            if (num < 8)
            {
                int length = (int)charArray.Length - 1;
                while (length >= (int)charArray.Length - num)
                {
                    if (charArray[length] == (char)num)
                    {
                        length--;
                    }
                    else
